...
export const OrganizerEvents: React.FC<OrganizerEventsProps> = ({ events, onNavigate, onEventsChanged, organizerWallet, eventsLoading = false, eventsError = null, startInCreate = false }) => {
  ...
  const submitForm = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (!localSaved) {
        setFeedback({ type: 'error', text: t('toasts.loadError') });
        return;
      }
  ...
  const confirmDelete = async () => {
    ...
    if (getStoredPurchasedTickets().some((ticket) => ticket.eventId === deleteTarget.id)) {
      setFeedback({ type: 'error', text: t('organizerEvents.cannotDeletePurchasedEvent') });
  ...
  return (
    <div className="min-h-screen py-8 sm:py-12 cyber-grid-bg text-left">
      <div className="mx-auto max-w-7xl space-y-6 px-4 sm:px-6 lg:px-8 animate-fadeIn">
        <header className="flex flex-col gap-4 border-b border-white/10 pb-6 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <button
              type="button"
              onClick={() => onNavigate('organizer')}
              className="mb-3 inline-flex min-h-11 items-center gap-2 text-xs font-semibold text-slate-400 hover:text-solana-cyan"
            >
              <ArrowLeft className="h-4 w-4" /> {t('common.back')}
            </button>
            <h1 className="text-2xl font-extrabold text-white sm:text-4xl">{t('organizerEvents.title')}</h1>
            <p className="mt-2 text-xs text-slate-300 sm:text-sm">{t('organizerEvents.subtitle')}</p>
          </div>
          <button
            type="button"
            onClick={() => { setEditingId(null); setForm(toForm()); }}
            className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-solana-purple to-neon-pink px-4 py-3 text-xs font-bold text-white shadow-lg active:scale-95"
          >
            <Plus className="h-4 w-4" /> {t('organizerEvents.createBtn')}
          </button>
        </header>
        ...
        <div className="flex flex-col gap-3 rounded-2xl border border-white/10 bg-[#120B30] p-4 sm:flex-row">
          <label className="relative flex-1">
            ...
            <input
              ...
              className="min-h-11 w-full rounded-xl border border-white/10 bg-black/30 pl-10 pr-3 text-sm text-white outline-none focus:border-solana-purple"
            />
          </label>
          <select
            value={statusFilter}
            onChange={(event) => setStatusFilter(event.target.value as 'all' | EventStatus)}
            className="min-h-11 rounded-xl border border-white/10 bg-[#171038] px-3 text-sm text-white outline-none"
          >
            <option value="all">{t('organizerEvents.allStatuses')}</option>
            <option value="published">{t('organizerEvents.published')}</option>
            <option value="draft">{t('organizerEvents.draft')}</option>
            <option value="cancelled">{t('organizerEvents.cancelled')}</option>
          </select>
        </div>

        {visibleEvents.length === 0 ? (
          <section className="rounded-2xl border border-white/10 bg-[#120B30] p-8 text-center">
            <CalendarDays className="mx-auto h-10 w-10 text-solana-cyan" />
            <h2 className="mt-3 text-xl font-bold text-white">{t('organizerEvents.noEvents')}</h2>
            <p className="mt-2 text-sm text-slate-400">{t('organizerEvents.noEventsHint')}</p>
          </section>
        ) : (
                ...
                <div className="space-y-4 p-5">
                  ...
                  <div className="grid grid-cols-2 gap-3 text-xs">
                    <div className="rounded-xl border border-white/10 bg-black/20 p-3">
                      <Package className="mb-2 h-4 w-4 text-solana-green" />
                      <span className="block text-slate-400">{t('organizerEvents.sold')}</span>
                      <strong className="text-white">{soldTickets(event)}</strong>
                    </div>
                    <div className="rounded-xl border border-white/10 bg-black/20 p-3">
                      <Package className="mb-2 h-4 w-4 text-solana-cyan" />
                      <span className="block text-slate-400">{t('organizerEvents.remaining')}</span>
                      <strong className="text-white">{remainingTickets(event)}</strong>
                    </div>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    <button
                      ...
                      className="inline-flex min-h-11 items-center gap-1.5 rounded-xl border border-white/10 px-3 text-xs font-semibold text-slate-200 hover:bg-white/5"
                    >
                      <Eye className="h-4 w-4" /> {t('organizerEvents.viewBtn')}
                    </button>
                    <button
                      type="button"
                      onClick={() => { setEditingId(event.id); setForm(toForm(event)); }}
                      className="inline-flex min-h-11 items-center gap-1.5 rounded-xl border border-solana-cyan/30 px-3 text-xs font-semibold text-solana-cyan hover:bg-solana-cyan/10"
                    >
                      <Edit3 className="h-4 w-4" /> {t('organizerEvents.editBtn')}
                    </button>
                    <button
                      type="button"
                      onClick={() => setDeleteTarget(event)}
                      className="inline-flex min-h-11 items-center gap-1.5 rounded-xl border border-neon-pink/30 px-3 text-xs font-semibold text-neon-pink hover:bg-neon-pink/10"
                    >
                      <Trash2 className="h-4 w-4" /> {t('organizerEvents.deleteBtn')}
                    </button>
                  </div>
                </div>
              </article>
        ...
        <p className="flex items-start gap-2 rounded-xl border border-solana-green/20 bg-solana-green/5 p-4 text-xs leading-relaxed text-slate-300">
          ...
          {t('organizerEvents.dbSyncNotice')}
        </p>
      </div>

      {form && (
        <div ref={createEventModalRef} className="fixed inset-x-0 bottom-0 top-16 z-[80] flex items-start justify-center overflow-y-auto bg-black/85 p-4 backdrop-blur-md sm:top-20">
          <form onSubmit={submitForm} className="relative my-4 w-full max-w-3xl space-y-5 rounded-2xl border border-solana-purple/40 bg-[#0F0A28] p-5 shadow-2xl sm:p-7">
            ...
            <div className="border-b border-white/10 pb-4 pr-12">
              <h2 className="text-xl font-bold text-white">
                {editingId ? t('organizerEvents.editTitle') : t('organizerEvents.createTitle')}
              </h2>
              <p className="mt-1 text-xs text-slate-400">
                {t('organizerEvents.dbBlockchainNotice')}
              </p>
            </div>
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              {([
                ['title', t('organizerEvents.eventName')],
                ['subtitle', t('organizerEvents.subtitleLabel')],
                ['venue', t('organizerEvents.venueLabel')],
                ['city', t('organizerEvents.cityLabel')],
                ['date', t('organizerEvents.dateLabel')],
                ['time', t('organizerEvents.timeLabel')],
                ['bannerImage', t('organizerEvents.bannerLabel')],
                ['thumbnailImage', t('organizerEvents.thumbnailLabel')]
              ] as const).map(([key, label]) => (
                <label key={key} className={key === 'title' || key === 'subtitle' || key === 'bannerImage' || key === 'thumbnailImage' ? 'sm:col-span-2' : ''}>
                  <span className="mb-1 block text-xs font-semibold text-slate-300">{label}</span>
                  <input
                    required={['title', 'venue', 'city', 'date', 'time'].includes(key)}
                    value={form[key]}
                    onChange={(event) => setForm({ ...form, [key]: event.target.value })}
                    className="min-h-11 w-full rounded-xl border border-white/10 bg-black/30 px-3 text-sm text-white outline-none focus:border-solana-purple"
                  />
                </label>
              ))}
              <label>
                <span className="mb-1 block text-xs font-semibold text-slate-300">{t('organizerEvents.categoryLabel')}</span>
                <select
                  value={form.category}
                  onChange={(event) => setForm({ ...form, category: event.target.value as Category })}
                  className="min-h-11 w-full rounded-xl border border-white/10 bg-[#171038] px-3 text-sm text-white"
                >
                  {categories.map((category) => <option key={category}>{category}</option>)}
                </select>
              </label>
              <label>
                <span className="mb-1 block text-xs font-semibold text-slate-300">{t('organizerEvents.statusLabel')}</span>
                <select
                  value={form.status}
              ...
              <label className="sm:col-span-2">
                <span className="mb-1 block text-xs font-semibold text-slate-300">{t('organizerEvents.descLabel')}</span>
                <textarea
                  required
                  value={form.description}
                  onChange={(event) => setForm({ ...form, description: event.target.value })}
                  rows={3}
                  className="w-full rounded-xl border border-white/10 bg-black/30 px-3 py-2 text-sm text-white outline-none focus:border-solana-purple"
                />
              </label>
            </div>
              ...
              {form.tiers.map((tier, index) => (
                <div key={tier.id} className="grid grid-cols-1 gap-2 rounded-xl border border-white/10 bg-black/20 p-3 sm:grid-cols-2">
                  <input
                    ...
                    placeholder={t('organizerEvents.tierName')}
                    className="min-h-11 rounded-xl border border-white/10 bg-black/30 px-3 text-sm text-white"
                  />
                  <input
                    type="number"
                    min="0"
                    step="0.01"
                    value={tier.priceSol}
                    onChange={(event) => updateTier(index, 'priceSol', Number(event.target.value))}
                    placeholder={t('organizerEvents.priceSol')}
                    className="min-h-11 rounded-xl border border-white/10 bg-black/30 px-3 text-sm text-white"
                  />
                  <input
                    type="number"
                    ...
                    onChange={(event) => updateTier(index, 'totalQuantity', Number(event.target.value))}
                    placeholder={t('organizerEvents.totalQty')}
                    className="min-h-11 rounded-xl border border-white/10 bg-black/30 px-3 text-sm text-white"
                  />
                  <input
                    type="number"
                    min="0"
                    value={tier.remainingQuantity}
                    onChange={(event) => updateTier(index, 'remainingQuantity', Number(event.target.value))}
                    placeholder={t('organizerEvents.remQty')}
                    className="min-h-11 rounded-xl border border-white/10 bg-black/30 px-3 text-sm text-white"
                  />
                  <input
                    value={tier.description}
                    onChange={(event) => updateTier(index, 'description', event.target.value)}
                    placeholder={t('organizerEvents.tierDesc')}
                    className="min-h-11 rounded-xl border border-white/10 bg-black/30 px-3 text-sm text-white sm:col-span-2"
                  />
                  <input
                    value={tier.perks}
                    onChange={(event) => updateTier(index, 'perks', event.target.value)}
            ...
            <div className="flex flex-col-reverse gap-3 border-t border-white/10 pt-4 sm:flex-row sm:justify-end">
              ...
              <button
                type="submit"
                disabled={isSaving}
                className="min-h-11 rounded-xl bg-gradient-to-r from-solana-purple to-neon-pink px-5 text-sm font-bold text-white disabled:cursor-wait disabled:opacity-60"
              >
                {isSaving ? t('organizerEvents.saving') : editingId ? t('organizerEvents.saveChanges') : t('organizerEvents.createBtn')}
              </button>
            </div>
          </form>
        </div>
      ...
      {viewing && (
        <div className="fixed inset-0 z-[75] flex items-center justify-center bg-black/85 p-4 backdrop-blur-md">
          <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl border border-solana-purple/40 bg-[#0F0A28] p-5 shadow-2xl">
            <div className="flex items-start justify-between gap-3">
              ...
              <button
                ...
                aria-label={t('common.close')}
                className="inline-flex h-11 w-11 items-center justify-center rounded-xl text-slate-300 hover:bg-white/10"
              >
                <X />
              </button>
            </div>
            ...
            <div className="mt-4 grid grid-cols-2 gap-3 text-xs">
              <div className="rounded-xl bg-black/20 p-3 text-slate-300">
                ...
                <strong className="mt-1 block text-white">{viewing.category}</strong>
              </div>
              <div className="rounded-xl bg-black/20 p-3 text-slate-300">
                Tickets
                <strong className="mt-1 block text-white">{soldTickets(viewing)} sold / {remainingTickets(viewing)} left</strong>
              </div>
            </div>
          </div>
        </div>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-[85] flex items-center justify-center bg-black/85 p-4 backdrop-blur-md">
          <div className="w-full max-w-md rounded-2xl border border-neon-pink/40 bg-[#0F0A28] p-6 shadow-2xl">
            <Trash2 className="h-8 w-8 text-neon-pink" />
            <h2 className="mt-4 text-xl font-bold text-white">{t('organizerEvents.deleteConfirmTitle')}</h2>
            <p className="mt-2 text-sm text-slate-300">{t('organizerEvents.deleteConfirmDesc')}</p>
            <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
              ...
              <button
                ...
                className="min-h-11 rounded-xl bg-neon-pink px-4 text-sm font-bold text-white disabled:cursor-wait disabled:opacity-60"
              >
                {isDeleting ? 'Deleting…' : t('organizerEvents.deleteBtn')}
              </button>
            </div>
          </div>
        </div>
